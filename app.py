...
def seed_baseline_data(conn):
    ...
    fallback_data = [
        ...
        ('CONAD', 'CONAD SOTTOGRUPPO', 'CONAD ADRIATICO', 'REFERENZA', '8002210001305', 54.00, None, None, None, None, None, 12.0, None, None, None, None, None, None, None, None),
    ]
    cursor.executemany("""
    ...
    if not gruppi:
        ...
        with col_l1:
            sconto_y = st.number_input(
                ...
                value=float(contract.sconto_6), step=0.5
            )
            st.markdown(
                f"<div class='warning-box'>ATTENZIONE - VALORE CONCORDATO A DATABASE: {contract.sconto_6:.2f}%<br>"
...
# ==========================================
    ...
    df_database_editor = pd.read_sql_query("""
        SELECT id, gruppo_macro, sottogruppo, associato_insegna, livello, chiave_livello, listino_r,
               sconto_1, sconto_2, sconto_3, sconto_4, sconto_5, sconto_6, sconto_7,
            ...
            with conn:
                ...
                for _, r in edited_df.iterrows():
                    ...
                    """, (
                        ...
                        check_nan(r.get("sconto_7")), check_nan(r.get("sconto_carico")), check_nan(r.get("sconto_pagamento")),
    ...
    with col_b1:
        ...
        SELECT a.gruppo_macro as GRUPPO_MACRO, a.sottogruppo as SOTTOGRUPPO, a.associato_insegna as ASSOCIATO_INSEGNA,
               ...
               a.sconto_6 as SCONTO_LOCAL_6, a.sconto_7 as SCONTO_LOCAL_7,
        ...
        colonne_ordinate = [
            ...
            "SCONTO_LOCAL_6", "SCONTO_LOCAL_7", "SCONTO_CARICO_LOGISTICA", "SCONTO_PAGAMENTO_AC",
                        ...
                        with conn:
                            for idx, row in df_import.iterrows():
                                ...
                                """, (
                                    ...
                                    to_float_or_none(row.get("SCONTO_LOCAL_7")), to_float_or_none(row.get("SCONTO_CARICO_LOGISTICA")),
        ...
        for p in all_prods:
            ...
            if resolved.listino_r is not None:
                input_calc = PricingInput(
                    ...
                    sconto_y=Decimal("0.00"), sconto_z=Decimal("0.00"), sconto_aa=Decimal("0.00"),
